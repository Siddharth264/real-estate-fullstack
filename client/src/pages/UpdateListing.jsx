  ...
  return (
    <main className="p-3 max-w-4xl mx-auto">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4 sm:flex-row">
        ...
        <p className="text-red-700 text-xs">{imageUploadError && <span className="font-extralight">{imageUploadError}</span>}</p>
        {
          formData.imageUrls.length > 0 && formData.imageUrls.map((url, index) => (
            <div key={url} className="flex justify-between p-3 border items-center">
              <img src={url} alt="listing image" className="w-20 h-20 object-contain rounded-lg" />
              <button type='button' onClick={()=>handleRemoveImage(index)} className="p-3 text-red-700 rounded-lg uppercase">Delete</button>
            </div>
          ))
        }
